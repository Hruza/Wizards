...
public class NetworkObjectPool : NetworkBehaviour
{
    ...
    List<PoolConfigObject> _pooledPrefabsList;
    List<PoolConfigObject> PooledPrefabsList{
        get{
            if(_pooledPrefabsList == null){
                _pooledPrefabsList = new List<PoolConfigObject>();
                foreach(NetworkPrefab prefab in prefabList.PrefabList){
                    _pooledPrefabsList.Add(new PoolConfigObject(){Prefab = prefab.Prefab,PrewarmCount=5});
                }
            }
            return _pooledPrefabsList;
        }
    }
    ...
    void RegisterPrefabInternal(GameObject prefab, int prewarmCount)
    {
        ...
        {
            NetworkObject no = Instantiate(prefab).GetComponent<NetworkObject>();
            return no;
        }
