...
public class DirectFire:Attack
{
    ...
    public override void OnStartAttack(Transform target,Transform source)
    {
        ...
        proj.GetComponent<Rigidbody>().velocity = (target.position - source.position).normalized * _element.standardVelocity;
...
public class CurvedFire:Attack
{
    ...
    public override void OnStartAttack(Transform target,Transform source)
    {
        ...
        proj.GetComponent<Rigidbody>().velocity = GetParabolicVelocity(source.position, target.position, _element.standardVelocity, projComponent.g , 50f);
